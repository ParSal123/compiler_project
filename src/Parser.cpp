...
Token *currentToken;
Scope *currentScope = new Scope(0, programIndex, memoryAddressAllocator, false, NORMAL, nullptr);
	...
	for (auto &path : diagrams[dfa])
	{
		    ...
		    (isNonTerminal(firstToken) && isInFirst(EPSILON_TOKEN_ID, firstToken) && isInFollow(tokenId, firstToken)))
		{
			...
			for (TokenId id : path)
			{
				...
				{
					if (!skipDirectives || tokenNames[id] == "#end_skip_directives")
					{
				...
				else
				{
					...
					{
						cerr << "token " << tokenNames[tokenId] << " is not in first or follow set of "
						     << tokenNames[id] << endl;
...
void Parser::initDirectiveFunctions()
{
	directiveFunctions = {
			...
			{tokenIndices["#add_param"],           add_param},
